...
export const useGetBooks = (initialOptions?: UseBooksOptions): UseBooksResult => {
  ...
  const [error, setError] = useState<string | null>(null)
  const [options, setOptions] = useState<UseBooksOptions>(
    initialOptions || { sortBy: 'title', sortOrder: 'asc' },
  )
  ...
  const fetchBooks = useCallback(
    async (
      currentSortBy: UseBooksOptions['sortBy'],
      currentSortOrder: UseBooksOptions['sortOrder'],
    ) => {
      setLoading(true)
      setError(null)
      try {
